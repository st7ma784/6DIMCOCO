...
class parser(HyperOptArgumentParser):
    def __init__(self,*args,strategy="random_search",**kwargs):
        ...
        self.opt_list("--lsa_version",default=list(get_all_LSA_fns().keys())[0],type=str,options=get_all_LSA_fns().keys())
        self.opt_list("--loss_version",default=list(get_loss_fns().keys())[0],type=str,options=get_loss_fns().keys())
